...
// Get DestinationID from GET parameter (or use a hidden field in the form)
$destinationID = isset($_GET['DestinationID']) ? $_GET['DestinationID'] : '';
if (empty($destinationID)) {
    die("Destination not specified.");
}

// Process the review submission if the request method is POST.
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get rating and comment from the form
    $rating = isset($_POST['rating']) ? intval($_POST['rating']) : 0;
    $comment = isset($_POST['comment']) ? trim($_POST['comment']) : '';

    // Insert the review into the "review" table.
    // The "Date" column is automatically set to CURRENT_TIMESTAMP, so we do not include it here.
    $queryReview = "INSERT INTO review (UserID, DestinationID, Rating, Comment) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($queryReview);
    if (!$stmt) {
        die("Prepare failed: " . $conn->error);
    }
    // "ssis" means: string, string, integer, string.
    $stmt->bind_param("ssis", $userID, $destinationID, $rating, $comment);
    $stmt->execute();
    if ($stmt->error) {
        die("Review insert error: " . $stmt->error);
    }
    // Get the auto-incremented ReviewID.
    $reviewID = $conn->insert_id;
    $stmt->close();

    // Process file upload if a photo was provided.
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['photo']['tmp_name'], $uploadPath)) {
            // Insert the file path into the "reviewimage" table.
            $queryImage = "INSERT INTO reviewimage (ReviewID, ReviewImage) VALUES (?, ?)";
            $stmt = $conn->prepare($queryImage);
            if (!$stmt) {
                die("Prepare failed: " . $conn->error);
            }
            // Although ReviewID is an integer in the review table,
            // the reviewimage table stores it as VARCHAR. We'll bind it as a string.
            $stmt->bind_param("is", $reviewID, $uploadPath);
            $stmt->execute();
            if ($stmt->error) {
                die("Review image insert error: " . $stmt->error);
            }
            $stmt->close();
        } else {
    ...
    // Redirect back to the destination page to prevent resubmission.
    header("Location: Destination.php?DestinationID=" . urlencode($destinationID));
    exit;
}
